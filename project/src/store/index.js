	...
	actions: {
		...
		getChamps() {
			async function getChar() {
				try {
					const Characters = await fetch("https://ddragon.leagueoflegends.com/cdn/12.6.1/data/en_US/champion.json").then(res => res.json());
					...
					async function getAccount() {
						try {
							...
							for (const matchID of apiAccount) {
								...
								const summonerInfo = apiMatch.info.participants.filter(summoner => summoner.summonerName === name)[0];
								// eslint-disable-next-line no-inner-declarations
								...
								const combos = [];
								const pos = [summonerInfo.doubleKills, summonerInfo.tripleKills, summonerInfo.quadraKills, summonerInfo.pentaKills];
								if (pos[3] != 0) {
